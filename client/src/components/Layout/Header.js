...
import { NavLink, useLocation, useNavigate, useNavigate } from "react-router-dom"
            ...
            <nav className="w-full flex justify-between items-center px-9 lg:w-large text-xs lg:text-base">
                ...
                <div className='flex gap-10'>
                    <ul className='md:flex gap-3 hidden'>
                        <li><button className="bg-sgreen-100 border-2 border-sgreen-100 text-xs text-white px-3 py-1 rounded-full" onClick={() => navigate("/signin")}><NavLink to='/signin'>Sign In</NavLink></button></li>
                        <li><button className="bg-white border-2 border-black text-xs text-black px-3 py-1 rounded-full" onClick={() => navigate("/signup")}><NavLink to='/signup'>Sign Up</NavLink></button></li>
                    </ul>
                </div>
